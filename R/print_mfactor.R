...
#' @export
print.mfactor <- function(x, quote = FALSE, max.levels = NULL, width = getOption("width"),...){
	# MOST OF THIS METHOD WAS COPIED FROM PRINT.FACTOR
    ord <- inherits(x,'ord_mfactor')
    if (length(x) == 0L) 
        cat(if (ord) "ord_mfactor" else "mfactor",
		   	"(0)\n", sep = "")
    else {
        xx <- x
        class(xx) <- NULL
        levels(xx) <- NULL
        xx[] <- as.character(x)
		attr(xx,'mlevels') <- NULL # strip the mlevels attribute
